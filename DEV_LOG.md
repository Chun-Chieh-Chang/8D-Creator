...
| 日期 | 版本 | 內容 |
|------|------|------|
| 2026-08-15 | v0.1.x | 行動裝置 (Mobile / Tablet) 響應式介面優化與 Drawer 導航重構：全面適配 375px~768px 行動螢幕；新增 Mobile Top Header Bar（漢堡選單、Logo、新建報告按鈕）；Sidebar 支援 Off-canvas Drawer 抽屜模式與遮罩點擊關閉；MainForm 表單由固定雙欄改為自適應單欄 (`grid-cols-1 sm:grid-cols-2`)，解決日期選擇器擠壓；5-Why 對話與即時預覽自適應小螢幕高度；報告完成頁操作按鈕列改為響應式 Flex-wrap；BrandSettingsPanel 與 AISettingsModal 彈窗優化配色網格 (`grid-cols-2 sm:grid-cols-3`)、按鈕排版與最大高度；全域字體維持 ≥14px，按鈕觸控區域提升至 ≥40-46px。TypeScript、ESLint 與 `npm run build` 零錯誤確效通過。（基準 Commit: `bc1f42a`） |
