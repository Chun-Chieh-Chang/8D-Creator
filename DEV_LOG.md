...
| 日期 | 版本 | 內容 |
|------|------|------|
| 2026-08-15 | v0.1.x | 多 Provider 模型適配整合：新增 `lib/ai_service.ts` 統一設定服務（Agnes 官方端點 + Google AI Studio Gemini 3.x/2.5/1.5 全系列 + 自訂 OpenAI-compatible 模型、Base URL 可設定、連線測試）；新增 `AISettingsModal` 進階設定（模型下拉 / API Key / Base URL / 測試連線）；agnes/gemini client 模型改為可設定（保留串流體驗）；Sidebar 快速切換與進階設定同步；相容既有 localStorage 設定；預設模型取自可選清單首位（Gemini 預設 gemini-3.7-flash） |
